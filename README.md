...
**Живой адрес:** <https://einsteinring.github.io/portfolio/>
...
│   └── previews/
│       ├── depo.png        # скриншоты демо, 1920×1200
│       ├── liteiny.png
│       └── shinapro.png
├── docs/
...
Из **этой** папки, где лежит `index.html`:

```bash
python -m http.server 5173
```

Если вы находитесь на уровень выше, в папке с проектами, укажите каталог явно:

```bash
python -m http.server 5173 --directory portfolio
```

Затем откройте <http://localhost:5173>. Остановить сервер — `Ctrl+C` в том же окне.
Альтернативы: `npx serve`, расширение Live Server в VS Code.

> Если браузер показывает «File not found» и в логе сервера видно `code 404`, значит сервер
> запущен не из той папки: он ищет `index.html` там, где его нет.
...
Форма проверяет поля на странице, а отправку делает функция `sendLead(data)` в
[`js/form.js`](js/form.js). Токен Telegram-бота в этот файл класть нельзя: GitHub Pages отдаёт
все файлы как есть, и любой посетитель прочитает их через «просмотр исходного кода».

Поэтому заявку принимает отдельный обработчик на Vercel, он лежит в соседней папке `lead-api`.
Токен хранится в переменных окружения Vercel и наружу не попадает.

```
браузер посетителя  →  POST /api/lead (Vercel)  →  Telegram Bot API  →  ваш чат
                          токен живёт здесь
```

Чтобы подключить форму, впишите адрес обработчика в начало [`js/form.js`](js/form.js):

```js
var LEAD_ENDPOINT = "https://lead-api-blue.vercel.app/api/lead";
```

Адрес уже вписан, форма работает. Если оставить строку пустой, форма честно сообщит, что не
подключена, и отправит человека в Telegram, вместо того чтобы показывать ложный успех.

Инструкция по развёртыванию обработчика: [`lead-api/README.md`](../lead-api/README.md).

В форме есть скрытое поле `company` — ловушка для ботов. Человек его не видит и не заполняет,
обработчик отвечает такому запросу успехом, но ничего не пересылает.
