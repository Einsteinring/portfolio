...
```js
var LEAD_ENDPOINT = "https://lead-api.vercel.app/api/lead";
```

Пока строка пустая, форма честно сообщает, что не подключена, и отправляет человека в Telegram.
Ложного «заявка отправлена» она не показывает — это важно, потому что сайт публичный.
